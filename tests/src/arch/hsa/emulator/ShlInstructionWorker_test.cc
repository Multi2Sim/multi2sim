...
#include <arch/hsa/emulator/ShlInstructionWorker.h>
...
TEST(ShlInstructionWorker, should_add_zero_at_least_significant_bits)
{
	// Setup environment
	...
	ShlInstructionWorker worker(work_item, nullptr);
	...
	retriever->return_value[0] = 0xFFFFFF0F;
	retriever->size[0] = 4;
	retriever->return_value[1] = 2;
	retriever->size[1] = 4;
	writer->expect_value = 0xFFFFFC3C;
	writer->size = 4;
