...
#define DEF_OPENCL_FUNC(_name, _argc) int evg_opencl_##_name##_impl(int *argv);
#include "opencl.dat"
#undef DEF_OPENCL_FUNC


/* Table of OpenCL function implementations */
typedef int (*evg_opencl_func_impl_t)(int *argv);
...
int evg_opencl_func_run(int code, int *argv)
{
	...
	if (code < EVG_OPENCL_FUNC_FIRST || code > EVG_OPENCL_FUNC_LAST)
		fatal("%s: invalid OpenCL function code - %d\n",
			__FUNCTION__, code);
	
	/* Debug */
	code -= EVG_OPENCL_FUNC_FIRST;
	evg_opencl_debug("%s\n", evg_opencl_func_name[code]);

	/* Call function */
	ret = evg_opencl_func_impl[code](argv);
	return ret;
}
...
int evg_opencl_clGetPlatformIDs_impl(int *argv)
{
...
int evg_opencl_clGetPlatformInfo_impl(int *argv)
{
...
int evg_opencl_clGetDeviceIDs_impl(int *argv)
{
...
int evg_opencl_clGetDeviceInfo_impl(int *argv)
{
	...
	size_ret = evg_opencl_device_get_info(device, param_name, x86_isa_mem,
...
int evg_opencl_clCreateContext_impl(int *argv)
{
...
int evg_opencl_clCreateContextFromType_impl(int *argv)
{
	unsigned int properties = argv[0];  /* const cl_context_properties *properties */
...
int evg_opencl_clReleaseContext_impl(int *argv)
{
...
int evg_opencl_clGetContextInfo_impl(int *argv)
{
	unsigned int context_id = argv[0];  /* cl_context context */
	...
	context = evg_opencl_repo_get_object(evg_emu->opencl_repo,
		evg_opencl_object_context, context_id);
	size_ret = evg_opencl_context_get_info(context, param_name, x86_isa_mem,
		param_value, param_value_size);
	if (param_value_size_ret)
		mem_write(x86_isa_mem, param_value_size_ret, 4, &size_ret);
...
int evg_opencl_clCreateCommandQueue_impl(int *argv)
{
	unsigned int context_id = argv[0];  /* cl_context context */
...
int evg_opencl_clReleaseCommandQueue_impl(int *argv)
{
...
int evg_opencl_clCreateBuffer_impl(int *argv)
{
	unsigned int context_id = argv[0];  /* cl_context context */
...
static struct string_map_t evg_opencl_create_image_flags_map =
{
	6, {
		{ "CL_MEM_READ_WRITE", 0x1 },
		{ "CL_MEM_WRITE_ONLY", 0x2 },
...
int evg_opencl_clCreateImage2D_impl(int *argv)
{
	unsigned int context_id = argv[0];  /* cl_context context */
	unsigned int flags = argv[1];  /* cl_mem_flags flags */
	...
	map_flags(&evg_opencl_create_image_flags_map, flags, flags_str, sizeof(flags_str));
...
int evg_opencl_clCreateImage3D_impl(int *argv)
{
	unsigned int context_id = argv[0];  /* cl_context context */
	unsigned int flags = argv[1];  /* cl_mem_flags flags */
	unsigned int image_format_ptr = argv[2];  /* cl_image_format *image_format */
...
int evg_opencl_clReleaseMemObject_impl(int *argv)
{
...
int evg_opencl_clCreateSampler_impl(int *argv)
{
...
int evg_opencl_clCreateProgramWithSource_impl(int *argv)
{
	unsigned int context_id = argv[0];  /* cl_context context */
...
int evg_opencl_clCreateProgramWithBinary_impl(int *argv)
{
	unsigned int context_id = argv[0];  /* cl_context context */
	unsigned int num_devices = argv[1];  /* cl_uint num_devices */
...
int evg_opencl_clReleaseProgram_impl(int *argv)
{
...
int evg_opencl_clBuildProgram_impl(int *argv)
{
	unsigned int program_id = argv[0];  /* cl_program program */
	unsigned int num_devices = argv[1];  /* cl_uint num_devices */
	unsigned int device_list = argv[2];  /* const cl_device_id *device_list */
...
int evg_opencl_clCreateKernel_impl(int *argv)
{
	unsigned int program_id = argv[0];  /* cl_program program */
...
int evg_opencl_clReleaseKernel_impl(int *argv)
{
...
int evg_opencl_clSetKernelArg_impl(int *argv)
{
	unsigned int kernel_id = argv[0];  /* cl_kernel kernel */
...
int evg_opencl_clGetKernelWorkGroupInfo_impl(int *argv)
{
	unsigned int kernel_id = argv[0];  /* cl_kernel kernel */
	unsigned int device_id = argv[1];  /* cl_device_id device */
...
int evg_opencl_clWaitForEvents_impl(int *argv)
{
...
int evg_opencl_clGetEventInfo_impl(int *argv)
{
...
int evg_opencl_clReleaseEvent_impl(int *argv)
{
...
int evg_opencl_clGetEventProfilingInfo_impl(int *argv)
{
	unsigned int event_id = argv[0];  /* cl_event event */
...
int evg_opencl_clFlush_impl(int *argv)
{
...
int evg_opencl_clEnqueueReadBuffer_impl(int *argv)
{
	unsigned int command_queue = argv[0];  /* cl_command_queue command_queue */
	unsigned int buffer = argv[1];  /* cl_mem buffer */
	unsigned int blocking_read = argv[2];  /* cl_bool blocking_read */
	unsigned int offset = argv[3];  /* size_t offset */
	unsigned int cb = argv[4];  /* size_t cb */
	unsigned int ptr = argv[5];  /* void *ptr */
	unsigned int num_events_in_wait_list = argv[6];  /* cl_uint num_events_in_wait_list */
	unsigned int event_wait_list = argv[7];  /* const cl_event *event_wait_list */
	unsigned int event_ptr = argv[8];  /* cl_event *event */
...
int evg_opencl_clEnqueueWriteBuffer_impl(int *argv)
{
	unsigned int command_queue = argv[0];  /* cl_command_queue command_queue */
	unsigned int buffer = argv[1];  /* cl_mem buffer */
	unsigned int blocking_write = argv[2];  /* cl_bool blocking_write */
	unsigned int offset = argv[3];  /* size_t offset */
	unsigned int cb = argv[4];  /* size_t cb */
	unsigned int ptr = argv[5];  /* const void *ptr */
	unsigned int num_events_in_wait_list = argv[6];  /* cl_uint num_events_in_wait_list */
	unsigned int event_wait_list = argv[7];  /* const cl_event *event_wait_list */
	unsigned int event_ptr = argv[8];  /* cl_event *event */
...
int evg_opencl_clEnqueueCopyBuffer_impl(int *argv)
{
	unsigned int command_queue = argv[0];  /* cl_command_queue command_queue */
	unsigned int src_buffer = argv[1];  /* cl_mem src_buffer */
	unsigned int dst_buffer = argv[2];  /* cl_mem dst_buffer */
	unsigned int src_offset = argv[3];  /* size_t src_offset */
	unsigned int dst_offset = argv[4];  /* size_t dst_offset */
	unsigned int cb = argv[5];  /* size_t cb */
	unsigned int num_events_in_wait_list = argv[6];  /* cl_uint num_events_in_wait_list */
	unsigned int event_wait_list = argv[7];  /* const cl_event *event_wait_list */
	unsigned int event_ptr = argv[8];  /* cl_event *event */
...
int evg_opencl_clEnqueueReadImage_impl(int *argv)
{
	unsigned int command_queue = argv[0];  /* cl_command_queue command_queue */
	unsigned int image = argv[1];  /* cl_mem image*/
	unsigned int blocking_read = argv[2];  /* cl_bool blocking_read */
	unsigned int origin = argv[3];  /* size_t origin[3] */
	unsigned int region = argv[4];  /* size_t region[3] */
	unsigned int row_pitch = argv[5];  /* size_t row_pitch */
	unsigned int slice_pitch = argv[6];  /* size_t slice_pitch */
	unsigned int ptr = argv[7];  /* void *ptr */
	unsigned int num_events_in_wait_list = argv[8];  /* cl_uint num_events_in_wait_list */
	unsigned int event_wait_list = argv[9];  /* const cl_event *event_wait_list */
	unsigned int event_ptr = argv[10];  /* cl_event *event */
...
int evg_opencl_clEnqueueMapBuffer_impl(int *argv)
{
	unsigned int command_queue = argv[0];  /* cl_command_queue command_queue */
	unsigned int buffer = argv[1];  /* cl_mem buffer */
	unsigned int blocking_map = argv[2];  /* cl_bool blocking_map */
	unsigned int map_flags = argv[3];  /* cl_map_flags map_flags */
	unsigned int offset = argv[4];  /* size_t offset */
	unsigned int cb = argv[5];  /* size_t cb */
	unsigned int num_events_in_wait_list = argv[6];  /* cl_uint num_events_in_wait_list */
	unsigned int event_wait_list = argv[7];  /* const cl_event *event_wait_list */
	unsigned int event_ptr = argv[8];  /* cl_event *event */
	unsigned int errcode_ret = argv[9];  /* cl_int *errcode_ret */
...
int evg_opencl_clEnqueueNDRangeKernel_impl(int *argv)
{
	unsigned int command_queue_id = argv[0];  /* cl_command_queue command_queue */
	unsigned int kernel_id = argv[1];  /* cl_kernel kernel */
	unsigned int work_dim = argv[2];  /* cl_uint work_dim */
	unsigned int global_work_offset_ptr = argv[3];  /* const size_t *global_work_offset */
	unsigned int global_work_size_ptr = argv[4];  /* const size_t *global_work_size */
	unsigned int local_work_size_ptr = argv[5];  /* const size_t *local_work_size */
	unsigned int num_events_in_wait_list = argv[6];  /* cl_uint num_events_in_wait_list */
	unsigned int event_wait_list = argv[7];  /* const cl_event *event_wait_list */
	unsigned int event_ptr = argv[8];  /* cl_event *event */
...
#define __EVG_OPENCL_NOT_IMPL__(_name) \
	int evg_opencl_##_name##_impl(int *argv) \
	{ \
