...
ShrInstructionWorker::ShrInstructionWorker(WorkItem *work_item,
...
ShrInstructionWorker::~ShrInstructionWorker()
{
}


template<typename T>
void ShrInstructionWorker::Inst_SHR_Aux(BrigCodeEntry *instruction)
{
	// Perform action
	T src0;
	unsigned int src1;
...
void ShrInstructionWorker::Execute(BrigCodeEntry *instruction)
{
	switch (instruction->getType())
	{
	case BRIG_TYPE_S32:

		Inst_SHR_Aux<int>(instruction);
		break;

	case BRIG_TYPE_S64:

		Inst_SHR_Aux<long long>(instruction);
		break;

	case BRIG_TYPE_U32:

		Inst_SHR_Aux<unsigned int>(instruction);
		break;

	case BRIG_TYPE_U64:

		Inst_SHR_Aux<unsigned long long>(instruction);
		break;
