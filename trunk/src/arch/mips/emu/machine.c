...
{
	int temp;
	temp = (int)MIPS_GPR_GET(RS) + (int)(OFFSET);
	MIPS_GPR_GET(RT) = temp;
}
...
void mips_isa_LH_impl(struct mips_ctx_t *ctx)
{
	...
	MIPS_GPR_SET(RT, SEXT32(temp, 16));
}
...
{
	unsigned short int temp = MIPS_GPR_GET(RT);
	unsigned int addr = MIPS_GPR_GET(RS) + IMM;
...
{
	unsigned int temp;
	mem_read(ctx->mem, MIPS_GPR_GET(RS) + IMM, sizeof(unsigned int), &temp);
	MIPS_GPR_SET(RT, temp);
...
void mips_isa_SWC1_impl(struct mips_ctx_t *ctx)
{
	...
	mem_write(ctx->mem, MIPS_GPR_GET(RS) + IMM, sizeof(unsigned int), &temp);
}
...
{
	MIPS_GPR_SET(RD, (int)MIPS_GPR_GET(RT) >> SA);
}
void mips_isa_SLLV_impl(struct mips_ctx_t *ctx)
{
	unsigned int s;
	s = BITS32(MIPS_GPR_GET(RS), 4, 0);
	MIPS_GPR_SET(RD, MIPS_GPR_GET(RT) << s);
}
void mips_isa_SRLV_impl(struct mips_ctx_t *ctx)
{
	unsigned int s = BITS32(MIPS_GPR_GET(RS), 4, 0);
	MIPS_GPR_SET(RD, MIPS_GPR_GET(RT) >> s);
}
...
{
	unsigned int s = BITS32(MIPS_GPR_GET(RS), 4, 0);
	MIPS_GPR_SET(RD, (int)MIPS_GPR_GET(RT) >> s);
}
...
{
	BRANCH(MIPS_GPR_GET(RS));
	MIPS_GPR_SET(RD, ctx->regs->pc + 8);
	mips_isa_inst_debug("  reg %d = pc + 8 = %x",RD, MIPS_GPR_GET(RD));
}
void mips_isa_MOVZ_impl(struct mips_ctx_t *ctx)
{
	if(MIPS_GPR_GET(RT) == 0)
		MIPS_GPR_SET(RD, MIPS_GPR_GET(RS));
}
...
{
	MIPS_GPR_SET(RD, MIPS_REG_LO);
}
void mips_isa_MTLO_impl(struct mips_ctx_t *ctx)
{
	ctx->regs->regs_LO = MIPS_GPR_GET(RS);
}
void mips_isa_MULT_impl(struct mips_ctx_t *ctx)
{
	unsigned long int temp = (long int) MIPS_GPR_GET((int)RS) *
			(long int) MIPS_GPR_GET((int)RT);
...
void mips_isa_DIV_impl(struct mips_ctx_t *ctx)
{
	if(MIPS_GPR_GET(RT))
	{
		ctx->regs->regs_LO = (int)MIPS_GPR_GET(RS) /
								(int)MIPS_GPR_GET(RT);
		ctx->regs->regs_HI = (int)MIPS_GPR_GET(RS) %
								(int)MIPS_GPR_GET(RT);
	}
}
void mips_isa_DIVU_impl(struct mips_ctx_t *ctx)
{
	if(MIPS_GPR_GET(RT))
	{
		ctx->regs->regs_LO = MIPS_GPR_GET(RS) /
								MIPS_GPR_GET(RT);
		ctx->regs->regs_HI = MIPS_GPR_GET(RS) %
								MIPS_GPR_GET(RT);
	}
}
void mips_isa_ADD_impl(struct mips_ctx_t *ctx)
{
	int temp = (int)MIPS_GPR_GET(RS) +
				(int)MIPS_GPR_GET(RT);
	MIPS_GPR_SET(RD, temp);
}
void mips_isa_ADDU_impl(struct mips_ctx_t *ctx)
{
	MIPS_GPR_SET(RD, MIPS_GPR_GET(RS) + MIPS_GPR_GET(RT));
	mips_isa_inst_debug("  %x = %x + %x", MIPS_GPR_GET(RD), MIPS_GPR_GET(RS), MIPS_GPR_GET(RT));
}
...
{
	unsigned int temp = MIPS_GPR_GET(RS) | MIPS_GPR_GET(RT);
	MIPS_GPR_SET(RD, ~temp);
}
void mips_isa_SLT_impl(struct mips_ctx_t *ctx)
{
	if ((int)MIPS_GPR_GET(RS) < (int)MIPS_GPR_GET(RT))
		MIPS_GPR_SET(RD, 1);
	else
		MIPS_GPR_SET(RD, 0);
}
...
{/*
	...
	temp1 = (int)MIPS_GPR_GET(RS);
	temp2 = (int)MIPS_GPR_GET(RT);
...
{
	long int temp = (long int) MIPS_GPR_GET((int)RS) *
					(long int) MIPS_GPR_GET((int)RT);
	MIPS_GPR_SET(RD, temp);
}
...
{
	/*	long int temp, temp1, temp2, temp3;
	temp1 = MIPS_GPR_GET((int)RS);
	temp2 = MIPS_GPR_GET((int)RT);
...
{
	MIPS_GPR_SET(RD, SEXT32((unsigned short int)MIPS_GPR_GET(RT), 16));
}
