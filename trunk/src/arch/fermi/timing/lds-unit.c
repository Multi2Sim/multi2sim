...
void frm_lds_write(struct frm_lds_t *lds)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		/* Access complete, remove the uop from the queue */
		uop->write_ready = frm_gpu->cycle + frm_gpu_lds_write_latency;
...
void frm_lds_read(struct frm_lds_t *lds)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		uop->read_ready = frm_gpu->cycle + frm_gpu_lds_read_latency;
...
void frm_lds_decode(struct frm_lds_t *lds)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		uop->decode_ready = frm_gpu->cycle + frm_gpu_lds_decode_latency;
