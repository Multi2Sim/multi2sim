...
{
	struct frm_ndrange_t *ndrange = thread_block->ndrange;
	...
	FRM_FOREACH_WAVEFRONT_IN_WORK_GROUP(thread_block, warp_id)
	{
		warp = ndrange->warps[warp_id];
		warp->id_in_sm = thread_block->id_in_sm *
			ndrange->warps_per_thread_block + 
	...
	int active_fb)
{
	...
	for (i = 0; i < frm_gpu_max_warps_per_warp_pool; i++)
	{
		...
		uop->inst = warp->inst;
		uop->cycle_created = frm_gpu->cycle;
		...
		if (frm_tracing())
		{
			frm_inst_dump(&warp->inst, warp->inst_size, 
				warp->pc, 
				warp->ndrange->inst_buffer + warp->pc,
		...
		uop->fetch_ready = frm_gpu->cycle + frm_gpu_fe_fetch_latency;
	...
	int active_fb)
{
		...
		issued_insts++)
	{
		for (i = 0; i < list_entries; i++)
		{
			 ...
			 * the fetch */
			if (frm_gpu->cycle < uop->fetch_ready)
			{
				list_index++;
				continue;
			}
			...
			{
				oldest_uop->issue_ready = frm_gpu->cycle + 
...
void frm_sm_run(struct frm_sm_t *sm)
{
	...
	active_fetch_buffer = frm_gpu->cycle % sm->num_warp_pools;
