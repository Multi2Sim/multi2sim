	...
	for (bidz = 0; bidz < function->group_count3[2]; bidz++)
	{
		for (bidy = 0; bidy < function->group_count3[1]; bidy++)
		{
			for (bidx = 0; bidx < function->group_count3[0]; bidx++)
			{
				...
				threadblock->thread_id_last = tid + function->local_size;
				...
				for (lidz = 0; lidz < function->local_size3[2]; lidz++)
				{
					for (lidy = 0; lidy < function->local_size3[1]; lidy++)
					{
						for (lidx = 0; lidx < function->local_size3[0]; lidx++)
						{
							...
							thread->gpr[1].elem[0] = bidx;  /* R1.x */
							thread->gpr[1].elem[1] = bidy;  /* R1.y */
							thread->gpr[1].elem[2] = bidz;  /* R1.z */
...
void frm_grid_setup_args(struct frm_grid_t *grid)
{
	...
	for (i = 0; i < list_count(function->arg_list); i++)
	{
		...
		if (!arg->set)
			fatal("function '%s': argument '%s' has not been assigned with 'clKernelSetArg'.",
		...
		if (arg->kind == FRM_CUDA_FUNCTION_ARG_KIND_POINTER)
		{
			...
			case FRM_CUDA_MEM_SCOPE_GLOBAL:
			{
				...
				//mem = frm_cuda_object_get(FRM_CUDA_OBJ_MEMORY, arg->value);
				//frm_isa_const_mem_write(1, cb_index, 0, &mem->device_ptr);
				//printf("    arg %d: cuda_mem id 0x%x loaded into CB1[%d]," 
			...
			case FRM_CUDA_MEM_SCOPE_LOCAL:
			{
				...
				//printf("    arg %d: %d bytes reserved in local memory at 0x%x\n",
