...
static void si_opencl_debug_array(int nelem, int *array)
{
	char *comma = "";
	int i;
...
char *err_si_opencl_binary_note =
...
char *err_si_opencl_version_note =
...
int si_opencl_func_run(int code, unsigned int *args)
{
	...
	assert(code >= SI_OPENCL_FUNC_FIRST && code <= SI_OPENCL_FUNC_LAST);
	func_code = code - SI_OPENCL_FUNC_FIRST;
	func_name = si_opencl_func_names[func_code];
	func_argc = si_opencl_func_argc[func_code];
	assert(func_argc <= SI_OPENCL_MAX_ARGS);
	...
	/* 1000 */
	case SI_OPENCL_FUNC_clGetPlatformIDs:
	{
		...
		if (opencl_impl_version < SI_SYS_OPENCL_IMPL_VERSION)
			fatal("wrong Multi2Sim OpenCL library version (provided=%d.%d.%d, required=%d.%d.%d).\n%s",
				...
				SI_SYS_OPENCL_IMPL_VERSION_MAJOR, SI_SYS_OPENCL_IMPL_VERSION_MINOR, SI_SYS_OPENCL_IMPL_VERSION_BUILD,
				err_si_opencl_version_note);
		...
		if (platforms && num_entries > 0)
			mem_write(x86_isa_mem, platforms, 4, &si_opencl_platform->id);
		break;
	}


	/* 1001 */
	case SI_OPENCL_FUNC_clGetPlatformInfo:
	{
		...
		platform = si_opencl_object_get(SI_OPENCL_OBJ_PLATFORM, platform_id);
		size_ret = si_opencl_platform_get_info(platform, param_name, x86_isa_mem, param_value, param_value_size);
	...
	/* 1002 */
	case SI_OPENCL_FUNC_clGetDeviceIDs:
	{
		...
		si_opencl_debug("  devices=0x%x, num_devices=%x\n",
			devices, num_devices);
		if (platform != si_opencl_platform->id)
		...
		if (devices && num_entries > 0) {
			if (!(device = (struct si_opencl_device_t *) si_opencl_object_get_type(SI_OPENCL_OBJ_DEVICE)))
	...
	/* 1003 */
	case SI_OPENCL_FUNC_clGetDeviceInfo:
	{
		...
		device = si_opencl_object_get(SI_OPENCL_OBJ_DEVICE, device_id);
		size_ret = si_opencl_device_get_info(device, param_name, x86_isa_mem, param_value, param_value_size);
		if (param_value_size_ret)
			mem_write(x86_isa_mem, param_value_size_ret, 4, &size_ret);
		break;
	}
	...
	/* 1004 */
	case SI_OPENCL_FUNC_clCreateContext:
	{
	...
	/* 1005 */
	case SI_OPENCL_FUNC_clCreateContextFromType:
	{
		uint32_t properties = args[0];  /* const cl_context_properties *properties */
		...
		/* Get device */
		device = (struct si_opencl_device_t *) si_opencl_object_get_type(SI_OPENCL_OBJ_DEVICE);
		assert(device);
	...
	/* 1007 */
	case SI_OPENCL_FUNC_clReleaseContext:
	{
		...
		context = si_opencl_object_get(SI_OPENCL_OBJ_CONTEXT, context_id);
	...
	/* 1008 */
	case SI_OPENCL_FUNC_clGetContextInfo:
	{
		uint32_t context_id = args[0];  /* cl_context context */
		...
		context = si_opencl_object_get(SI_OPENCL_OBJ_CONTEXT, context_id);
		size_ret = si_opencl_context_get_info(context, param_name, x86_isa_mem, param_value, param_value_size);
		if (param_value_size_ret)
			mem_write(x86_isa_mem, param_value_size_ret, 4, &size_ret);
		break;
	}
	...
	/* 1009 */
	case SI_OPENCL_FUNC_clCreateCommandQueue:
	{
		uint32_t context_id = args[0];  /* cl_context context */
		...
		si_opencl_object_get(SI_OPENCL_OBJ_CONTEXT, context_id);
		si_opencl_object_get(SI_OPENCL_OBJ_DEVICE, device_id);
	...
	/* 1011 */
	case SI_OPENCL_FUNC_clReleaseCommandQueue:
	{
		...
		command_queue = si_opencl_object_get(SI_OPENCL_OBJ_COMMAND_QUEUE, command_queue_id);
	...
	/* 1014 */
	case SI_OPENCL_FUNC_clCreateBuffer:
	{
		uint32_t context_id = args[0];  /* cl_context context */
		...
		if ((flags & 0x10) && host_ptr)
			fatal("%s: CL_MEM_ALLOC_HOST_PTR not compatible with CL_MEM_USE_HOST_PTR\n%s",
				err_prefix, err_si_opencl_param_note);
	...
	/* 1016 */
	case SI_OPENCL_FUNC_clCreateImage2D:
	{
		uint32_t context_id = args[0];  /* cl_context context */
		uint32_t flags = args[1];  /* cl_mem_flags flags */
		...
		si_opencl_debug("  context=0x%x, flags=%s, channel order =0x%x, channel_type=0x%x, image_width=%u, image_height=%u, image_row_pitch=%u, host_ptr=0x%x, errcode_ret=0x%x\n",
		...
		default:
		{
			fatal("%s: image channel order %u not supported\n%s",
				err_prefix, channel_order, err_si_opencl_param_note);
		}
		...
		default:
		{
			fatal("%s: image channel type %u not supported\n%s",
				err_prefix, channel_type, err_si_opencl_param_note);
		}
	...
	/* 1017 */
	case SI_OPENCL_FUNC_clCreateImage3D:
	{
		uint32_t context_id = args[0];  /* cl_context context */
		uint32_t flags = args[1];  /* cl_mem_flags flags */
		uint32_t image_format_ptr = args[2];  /* cl_image_format *image_format */
	...
	/* 1019 */
	case SI_OPENCL_FUNC_clReleaseMemObject:
	{
		...
		mem = si_opencl_object_get(SI_OPENCL_OBJ_MEM, mem_id);
	...
	/* 1024 */
	case SI_OPENCL_FUNC_clCreateSampler:
	{
		...
		si_opencl_object_get(SI_OPENCL_OBJ_CONTEXT, context);
	...
	/* 1028 */
	case SI_OPENCL_FUNC_clCreateProgramWithSource:
	{
		uint32_t context_id = args[0];  /* cl_context context */
		...
		if (!*si_emu_opencl_binary_name)
			fatal("%s: kernel source compilation not supported.\n%s",
				err_prefix, err_si_opencl_compiler);
		...
		warning("%s: binary '%s' used as pre-compiled kernel.\n%s",
			err_prefix, si_emu_opencl_binary_name, err_si_opencl_binary_note);
	...
	/* 1029 */
	case SI_OPENCL_FUNC_clCreateProgramWithBinary:
	{
		uint32_t context_id = args[0];  /* cl_context context */
		uint32_t num_devices = args[1];  /* cl_uint num_devices */
	...
	/* 1031 */
	case SI_OPENCL_FUNC_clReleaseProgram:
	{
		...
		program = si_opencl_object_get(SI_OPENCL_OBJ_PROGRAM, program_id);
	...
	/* 1032 */
	case SI_OPENCL_FUNC_clBuildProgram:
	{
		uint32_t program_id = args[0];  /* cl_program program */
		uint32_t num_devices = args[1];  /* cl_uint num_devices */
		uint32_t device_list = args[2];  /* const cl_device_id *device_list */
	...
	/* 1036 */
	case SI_OPENCL_FUNC_clCreateKernel:
	{
		uint32_t program_id = args[0];  /* cl_program program */
	...
	/* 1039 */
	case SI_OPENCL_FUNC_clReleaseKernel:
	{
		...
		kernel = si_opencl_object_get(SI_OPENCL_OBJ_KERNEL, kernel_id);
	...
	/* 1040 */
	case SI_OPENCL_FUNC_clSetKernelArg:
	{
		uint32_t kernel_id = args[0];  /* cl_kernel kernel */
	...
	/* 1042 */
	case SI_OPENCL_FUNC_clGetKernelWorkGroupInfo:
	{
		uint32_t kernel_id = args[0];  /* cl_kernel kernel */
		uint32_t device_id = args[1];  /* cl_device_id device */
	...
	/* 1043 */
	case SI_OPENCL_FUNC_clWaitForEvents:
	{
	...
	/* 1044 */
	case SI_OPENCL_FUNC_clGetEventInfo:
	{
	...
	/* 1047 */
	case SI_OPENCL_FUNC_clReleaseEvent:
	{
		...
		event = si_opencl_object_get(SI_OPENCL_OBJ_EVENT, event_id);
	...
	/* 1050 */
	case SI_OPENCL_FUNC_clGetEventProfilingInfo:
	{
		uint32_t event_id = args[0];  /* cl_event event */
	...
	/* 1053 */
	case SI_OPENCL_FUNC_clEnqueueReadBuffer:
	{
		uint32_t command_queue = args[0];  /* cl_command_queue command_queue */
		...
		SI_OPENCL_ARG_NOT_SUPPORTED_NEQ(num_events_in_wait_list, 0);
		SI_OPENCL_ARG_NOT_SUPPORTED_NEQ(event_wait_list, 0);

		/* Get memory object */
		mem = si_opencl_object_get(SI_OPENCL_OBJ_MEM, buffer);
		...
		if (!buf)
			fatal("out of memory");
		mem_read(si_emu->global_mem, mem->device_ptr + offset, cb, buf);
		...
		/* Return success */
		si_opencl_debug("    %d bytes copied from device memory (0x%x) to host memory (0x%x)\n",
	...
	/* 1055 */
	case SI_OPENCL_FUNC_clEnqueueWriteBuffer:
	{
		uint32_t command_queue = args[0];  /* cl_command_queue command_queue */
		uint32_t buffer = args[1];  /* cl_mem buffer */
		...
		mem_write(si_emu->global_mem, mem->device_ptr + offset, cb, buf);
		free(buf);
		...
		/* Return success */
		si_opencl_debug("    %d bytes copied from host memory (0x%x) to device memory (0x%x)\n",
	...
	/* 1057 */
	case SI_OPENCL_FUNC_clEnqueueCopyBuffer:
	{
		uint32_t command_queue = args[0];  /* cl_command_queue command_queue */
		...
		/* Get memory objects */
		src_mem = si_opencl_object_get(SI_OPENCL_OBJ_MEM, src_buffer);
		dst_mem = si_opencl_object_get(SI_OPENCL_OBJ_MEM, dst_buffer);
		...
		if (!buf)
			fatal("out of memory");
		mem_read(si_emu->global_mem, src_mem->device_ptr + src_offset, cb, buf);
		mem_write(si_emu->global_mem, dst_mem->device_ptr + dst_offset, cb, buf);
		free(buf);
		...
		/* Return success */
		si_opencl_debug("    %d bytes copied in device memory from 0x%x to 0x%x\n",
			cb, src_mem->device_ptr + src_offset, dst_mem->device_ptr + dst_offset);
		break;
	}

	/* 1059 */ 
	case SI_OPENCL_FUNC_clEnqueueReadImage:
	{
		uint32_t command_queue = args[0];  /* cl_command_queue command_queue */
		...
		/* Get memory object */
		mem = si_opencl_object_get(SI_OPENCL_OBJ_MEM, image);
	...
	/* 1064 */
	case SI_OPENCL_FUNC_clEnqueueMapBuffer:
	{
		uint32_t command_queue = args[0];  /* cl_command_queue command_queue */
		uint32_t buffer = args[1];  /* cl_mem buffer */
		...
		SI_OPENCL_ARG_NOT_SUPPORTED_NEQ(num_events_in_wait_list, 0);
		SI_OPENCL_ARG_NOT_SUPPORTED_NEQ(event_wait_list, 0);
		SI_OPENCL_ARG_NOT_SUPPORTED_EQ(blocking_map, 0);

		/* Get memory object */
		si_opencl_object_get(SI_OPENCL_OBJ_MEM, buffer);
	...
	/* 1067 */
	case SI_OPENCL_FUNC_clEnqueueNDRangeKernel:
	{
		uint32_t command_queue = args[0];  /* cl_command_queue command_queue */
		...
		SI_OPENCL_ARG_NOT_SUPPORTED_NEQ(global_work_offset_ptr, 0);
		SI_OPENCL_ARG_NOT_SUPPORTED_RANGE(work_dim, 1, 3);
		...
		si_opencl_debug_array(work_dim, kernel->local_size3);
		si_opencl_debug("\n");
		...
		si_opencl_debug_array(work_dim, kernel->group_count3);
		si_opencl_debug("\n");
		...
		/* Setup NDRange */
		kernel->ndrange = si_ndrange_create(kernel);
		si_ndrange_setup_work_items(kernel->ndrange);
		si_ndrange_setup_const_mem(kernel->ndrange);
		si_ndrange_setup_args(kernel->ndrange);

		/* Launch kernel execution */
		if (si_emu_kind == si_emu_functional)
		...
		/* Free NDRange */
		si_ndrange_free(kernel->ndrange);
	...
	default:
		...
		fatal("SI_OPENCL_FUNC_run: function '%s' (code=%d) not implemented.\n%s",
			func_name, code, err_si_opencl_note);
	}
