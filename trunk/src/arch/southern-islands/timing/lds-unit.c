...
int si_gpu_lds_exec_latency = 1;
int si_gpu_lds_exec_buffer_size = 1;
...
void si_lds_write(struct si_lds_t *lds)
{
	...
	assert(list_entries <= si_gpu_lds_exec_buffer_size);
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		uop->write_ready = si_gpu->cycle + si_gpu_lds_write_latency;
		list_remove(lds->exec_buffer, uop);
		list_enqueue(lds->write_buffer, uop);

		instructions_processed++;

        si_trace("si.inst id=%lld cu=%d wf=%d uop_id=%lld stg=\"lds-w\"\n", 
            uop->id_in_compute_unit, lds->compute_unit->id, 
            uop->wavefront->id, uop->id_in_wavefront);
	}
}
...
void si_lds_execute(struct si_lds_t *lds)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		assert(list_count(lds->exec_buffer) <= si_gpu_lds_exec_buffer_size);
		...
		if (list_count(lds->inflight_buffer) < si_gpu_lds_inflight_mem_accesses &&
                list_count(lds->exec_buffer) < si_gpu_lds_exec_buffer_size)
		{
			...
			uop->execute_ready = si_gpu->cycle + si_gpu_lds_exec_latency;
...
void si_lds_decode(struct si_lds_t *lds)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		uop->decode_ready = si_gpu->cycle + si_gpu_lds_decode_latency;
		list_remove(lds->issue_buffer, uop);
		list_enqueue(lds->decode_buffer, uop);

		instructions_processed++;

        si_trace("si.inst id=%lld cu=%d wf=%d uop_id=%lld stg=\"lds-d\"\n", 
            uop->id_in_compute_unit, lds->compute_unit->id, 
            uop->wavefront->id, uop->id_in_wavefront);
	}
}
