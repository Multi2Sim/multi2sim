...
void si_lds_write(struct si_lds_t *lds)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		/* Access complete, remove the uop from the queue */
		uop->write_ready = arch->cycle_count + si_gpu_lds_write_latency;
...
void si_lds_read(struct si_lds_t *lds)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		uop->read_ready = arch->cycle_count + si_gpu_lds_read_latency;
...
void si_lds_decode(struct si_lds_t *lds)
{
	...
	for (i = 0; i < list_entries; i++)
	{
		...
		uop->decode_ready = arch->cycle_count + si_gpu_lds_decode_latency;
