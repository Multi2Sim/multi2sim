...
void Context::Parse(const std::string &in, const std::string &out)
{
	...
	for (auto &llvm_function : llvm_module->getFunctionList())
	{
		...
		// Ignore built-in functions. Built-in function by Clang front-end
		// has no attribute NoUnWind, but sometimes with ReadOnly.
		if (!llvm_function.hasFnAttribute(llvm::Attribute::NoUnwind) ||
			llvm_function.hasFnAttribute(llvm::Attribute::ReadOnly))
			continue;
