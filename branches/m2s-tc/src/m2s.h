...
struct uop_t {
	...
	//x86_inst_t *inst;
	struct ctx_t *ctx;
	int size;  /* Macroinstruction size */
	int core, thread;
	int specmode;
...
/* Branch kind */
enum btb_branch_kind_enum {
	btb_branch_kind_branch,
	btb_branch_kind_call,
	btb_branch_kind_ret
};
...
struct btb_entry_t {
	uint32_t source;  /* eip */
	uint32_t dest;  /* neip */
	int size;  /* size of instruction - needed to stack return address */
	enum btb_branch_kind_enum kind;
};
...
struct bpred_t {
	...
	/* BTB - array of bpred_btb_sets * bpred_btb_ways */
	struct btb_entry_t *btb;

	/* GAg predictor */
	uint32_t twolev_bhr;
	char *twolev_pht;  /* array of 2^bpred_bhr_size 2-bit counters */
...
void p_writeback();
void p_commit();
void p_recover(struct uop_t *uop);
