...
static struct net_buffer_t *net_photo_link_arbiteration_per_wavelength(struct net_node_t
	*bus_node, struct net_bus_t *bus)
{
	struct net_buffer_t *src_buffer;
	struct net_packet_t *msg;
	...
	struct net_buffer_t *buffer)
{
	...
	assert(buffer->bus);
	assert(!buffer->link);
	assert(bus_node->kind == net_node_photonic);

	for (i = 0; i < list_count(bus_node->bus_lane_list); i++)
	{
		check_bus = list_get(bus_node->bus_lane_list, (i + bus_node->last_lane_index + 1) %
				list_count(bus_node->bus_lane_list));
		if (net_photo_link_arbiteration_per_wavelength(bus_node,
				check_bus) == buffer)
		{
			bus_node->last_lane_index = (i + bus_node->last_lane_index + 1) %
					list_count(bus_node->bus_lane_list);
			return check_bus;
		}
