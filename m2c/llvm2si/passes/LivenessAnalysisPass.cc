...
{
	// Run liveness Analysis pass first for scalar registers
	// and then vector registers.
	if(getPassType() == 1)
		runScalar();
...
void LivenessAnalysisPass::runScalar()
{
			...
			basic_block++)
	{
							...
							instruction++)
		{
			...
			for (auto &argument : (*instruction)->getArguments())
			{
				...
				{ // destination ends the liveness of a variable
						...
						switch (argument->getType())
						{
							...
							case si2bin::Argument::TypeScalarRegisterSeries:
							{
								...
								while (low <= high)
								{
									s_use.Set(low, false);
									s_def.Set(low, true);
									low++; // go to next register in the series
								}
							break;
							}
							default:
					...
					{
						switch (argument->getType())
						{ // source begins the liveness of a variable
							case si2bin::Argument::TypeScalarRegister:
							{
								scalar_register = misc::cast<si2bin::ArgScalarRegister*>(argument.get());
	...
	misc::Bitmap s_in(lap_function->getNumScalarRegisters());
	misc::Bitmap s_out(lap_function->getNumScalarRegisters());
	...
	while(changed == true)
	{
					...
					++basic_block_current)
		{
			...
			if (s_in == s_in_current && s_out == s_out_current)
...
void LivenessAnalysisPass::runVector()
{
	// Create a basic block queue
	std::vector<llvm2si::BasicBlock *> basic_block_queue;

	//Create use and def set for
			...
			basic_block++)
	{
		...
		// create pointer to vector register
		...
		while(changed == true)
		{
						...
						++basic_block_current)
			{
				...
				// The check must continue until the live in and live out of every
				// basic block does not change anymore
				if (v_in == v_in_current && v_out == v_out_current)
					bb_change.push_back(false);
				else
					bb_change.push_back(true);
			}
