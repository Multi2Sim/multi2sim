...
int opencl_func_run(int code, unsigned int *args)
{
	...
	case OPENCL_FUNC_clCreateBuffer:
	{
		...
		/* Check flags */
		if (flags & 0x10)
			fatal("%s: flag 'CL_MEM_ALLOC_HOST_PTR' not supported\n%s",
				err_prefix, err_opencl_note);
		/* FIXME check flags */
	...
	{
		uint32_t command_queue = args[0];  /* cl_command_queue command_queue */
		uint32_t kernel = args[1];  /* cl_kernel kernel */
		...
		opencl_debug("  command_queue=0x%x, kernel=0x%x, work_dim=0x%x,\n"
		...
		OPENCL_PARAM_NOT_SUPPORTED_NEQ(local_work_size_ptr, 0);
		OPENCL_PARAM_NOT_SUPPORTED_NEQ(work_dim, 1);

		/* Get global work size */
		mem_read(isa_mem, global_work_size_ptr, 4, &global_work_size);
		opencl_debug("    global_work_size=%d\n", global_work_size);
