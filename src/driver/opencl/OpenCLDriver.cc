...
int OpenCLSIDriver::DriverCall(x86::Context *ctx)
{
	// const x86::Regs &regs = ctx->getRegs();

	// // Variables

	// // Function code
	// int code = (int)regs.getEax();
	// if (code <= OpenCLABIInvalid || code >= OpenCLABICallCount)
	// 	fatal("%s: invalid OpenCL ABI call (code %d).\n%s",
	// 		__FUNCTION__, code, OpenCLErrABICall);

	// // Debug
	// x86::Emu::call_debug << misc::fmt("  %s (code %d)\n", 
	// 	OpenCLABICallName[code].c_str(), code);
	// x86::Emu::opencl_debug << misc::fmt("OpenCL ABI call '%s' (code %d)\n",
	// 	OpenCLABICallName[code].c_str(), code);
