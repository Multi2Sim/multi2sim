...
static int x86_cpu_can_fetch(int core, int thread)
{
	...
	if (X86_THREAD.fetch_stall_until >= asTiming(x86_cpu)->cycle || ctx->evict_signal)
		return 0;
...
static struct x86_uop_t *x86_cpu_fetch_inst(int core, int thread, int fetch_trace_cache)
{
	...
	X86ContextSetEip(ctx, X86_THREAD.fetch_eip);
...
static int x86_cpu_fetch_thread_trace_cache(int core, int thread)
{
	...
	for (i = 0; i < mop_count; i++)
	{
		...
		if (!X86ContextGetState(X86_THREAD.ctx, X86ContextRunning))
			break;
...
static void x86_cpu_fetch_core(int core)
{
	...
	case x86_cpu_fetch_kind_switchonevent:
	{
		...
		if (X86_THREAD.fetch_stall_until >= asTiming(x86_cpu)->cycle)
			break;
		...
		must_switch = must_switch || asTiming(x86_cpu)->cycle - X86_CORE.fetch_switch_when >
		...
		if (must_switch)
		{
			...
			if (new != thread)
			{
				...
				X86_CORE.fetch_switch_when = asTiming(x86_cpu)->cycle;
				X86_THREAD_IDX(new).fetch_stall_until = asTiming(x86_cpu)->cycle + x86_cpu_thread_switch_penalty - 1;
			}
		}
