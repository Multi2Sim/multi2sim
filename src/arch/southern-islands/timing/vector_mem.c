...
void si_vector_mem_read(struct si_vector_mem_unit_t *vector_mem)
{
	...
	list_count = linked_list_count(vector_mem->read_inst_buffer);

	/* No instruction to process */
	if (!list_count)
		return;

	linked_list_head(vector_mem->read_inst_buffer);
	uop = linked_list_get(vector_mem->read_inst_buffer);
	linked_list_remove(vector_mem->read_inst_buffer);
