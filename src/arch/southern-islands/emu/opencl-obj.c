...
struct linked_list_t *si_opencl_object_list;


/* Add an OpenCL object to object list */
void si_opencl_object_add(void *object)
{
	linked_list_find(si_opencl_object_list, object);
	assert(si_opencl_object_list->error_code);
	linked_list_add(si_opencl_object_list, object);
}
...
void si_opencl_object_remove(void *object)
{
	linked_list_find(si_opencl_object_list, object);
	assert(!si_opencl_object_list->error_code);
	linked_list_remove(si_opencl_object_list);
}
...
void *si_opencl_object_get(enum si_opencl_obj_t type, uint32_t id)
{
	...
	if (id >> 16 != type)
		fatal("%s: requested OpenCL object of incorrect type",
			__FUNCTION__);
	LINKED_LIST_FOR_EACH(si_opencl_object_list)
	{
		if (!(object = linked_list_get(si_opencl_object_list)))
...
void si_opencl_object_free_all()
{
	void *object;

	/* Platforms */
	while ((object = si_opencl_object_get_type(SI_OPENCL_OBJ_PLATFORM)))
		si_opencl_platform_free((struct si_opencl_platform_t *) object);
	
	/* Devices */
	while ((object = si_opencl_object_get_type(SI_OPENCL_OBJ_DEVICE)))
		si_opencl_device_free((struct si_opencl_device_t *) object);
	
	/* Contexts */
	while ((object = si_opencl_object_get_type(SI_OPENCL_OBJ_CONTEXT)))
		si_opencl_context_free((struct si_opencl_context_t *) object);
	
	/* Command queues */
	while ((object = si_opencl_object_get_type(SI_OPENCL_OBJ_COMMAND_QUEUE)))
		si_opencl_command_queue_free((struct si_opencl_command_queue_t *) object);
	
	/* Programs */
	while ((object = si_opencl_object_get_type(SI_OPENCL_OBJ_PROGRAM)))
		si_opencl_program_free((struct si_opencl_program_t *) object);
	
	/* Kernels */
	while ((object = si_opencl_object_get_type(SI_OPENCL_OBJ_KERNEL)))
		si_opencl_kernel_free((struct si_opencl_kernel_t *) object);
	
	/* Mems */
	while ((object = si_opencl_object_get_type(SI_OPENCL_OBJ_MEM)))
		si_opencl_mem_free((struct si_opencl_mem_t *) object);
	
	/* Events */
	while ((object = si_opencl_object_get_type(SI_OPENCL_OBJ_EVENT)))
		si_opencl_event_free((struct si_opencl_event_t *) object);

	/* Samplers */
	while ((object = si_opencl_object_get_type(SI_OPENCL_OBJ_SAMPLER)))
		si_opencl_sampler_free((struct si_opencl_sampler_t *) object);
	
	/* Any object left */
	if (linked_list_count(si_opencl_object_list))
...
struct si_opencl_platform_t *si_opencl_platform;
...
struct si_opencl_platform_t *si_opencl_platform_create()
{
	...
	/* Initialize */
	platform->id = si_opencl_object_new_id(SI_OPENCL_OBJ_PLATFORM);

	/* Return */
	si_opencl_object_add(platform);
	return platform;
}
...
{
	si_opencl_object_remove(platform);
	free(platform);
}
...
struct si_opencl_device_t *si_opencl_device_create()
{
	...
	/* Initialize */
	device->id = si_opencl_object_new_id(SI_OPENCL_OBJ_DEVICE);

	/* Return */
	si_opencl_object_add(device);
	return device;
}
...
{
	si_opencl_object_remove(device);
	free(device);
}
...
struct si_opencl_context_t *si_opencl_context_create()
{
	...
	/* Initialize */
	context->id = si_opencl_object_new_id(SI_OPENCL_OBJ_CONTEXT);
	context->ref_count = 1;

	/* Return */
	si_opencl_object_add(context);
	return context;
}
...
{
	si_opencl_object_remove(context);
	free(context);
}
...
void si_opencl_context_set_properties(struct si_opencl_context_t *context, struct mem_t *mem, uint32_t addr)
{
	...
	while (addr)
	{
		...
		case 0x1084:  /* CL_CONTEXT_PLATFORM */
			...
			si_opencl_object_get(SI_OPENCL_OBJ_PLATFORM, value);
...
struct si_opencl_sampler_t *si_opencl_sampler_create()
{
	...
	/* Initialize */
	sampler->id = si_opencl_object_new_id(SI_OPENCL_OBJ_SAMPLER);
	sampler->ref_count = 1;

	/* Return */
	si_opencl_object_add(sampler);
	return sampler;
}
...
{
	si_opencl_object_remove(sampler);
	free(sampler);
}
...
struct si_opencl_command_queue_t *si_opencl_command_queue_create()
{
	...
	/* Initialize */
	command_queue->id = si_opencl_object_new_id(SI_OPENCL_OBJ_COMMAND_QUEUE);
	command_queue->ref_count = 1;

	/* Return */
	si_opencl_object_add(command_queue);
...
{
	si_opencl_object_remove(command_queue);
	free(command_queue);
}
...
struct si_opencl_program_t *si_opencl_program_create()
{
	...
	/* Initialize */
	program->id = si_opencl_object_new_id(SI_OPENCL_OBJ_PROGRAM);
	program->ref_count = 1;
...
void si_opencl_program_free(struct si_opencl_program_t *program)
{
	...
	si_opencl_object_remove(program);
	free(program);
}
...
static char *err_si_opencl_southern_islands_format =
	...
	"\tdoes not contain code in the Southern Islands ISA. Please, check that the off-line\n"
...
struct si_opencl_kernel_t *si_opencl_kernel_create()
{
	...
	/* Initialize */
	kernel->id = si_opencl_object_new_id(SI_OPENCL_OBJ_KERNEL);
	kernel->ref_count = 1;
...
void si_opencl_kernel_free(struct si_opencl_kernel_t *kernel)
{
	...
	/* Free kernel */
	si_opencl_object_remove(kernel);
	free(kernel);
}
...
#define SI_OPENCL_KERNEL_METADATA_NOT_SUPPORTED(_idx) \
	...
	err_si_opencl_kernel_metadata_note);
...
static char *err_si_opencl_kernel_metadata_note =
		...
		if (!strcmp(line_ptrs[0], "image"))
		{
			...
			else
			{
				fatal("%s: Invalid number of dimensions for OpenCL Image (%s)\n%s",
					__FUNCTION__, line_ptrs[2], err_si_opencl_param_note);
			}
			...
			else
			{
				fatal("%s: Invalid memory access type for OpenCL Image (%s)\n%s",
					__FUNCTION__, line_ptrs[3], err_si_opencl_param_note);
			}
...
void si_opencl_kernel_load(struct si_opencl_kernel_t *kernel, char *kernel_name)
{
	...
	program = si_opencl_object_get(SI_OPENCL_OBJ_PROGRAM, kernel->program_id);
	...
	struct mem_t *mem, uint32_t addr, uint32_t size)
{
	...
	default:
		fatal("%s: invalid or not implemented value for 'name' (0x%x)\n%s",
			__FUNCTION__, name, err_si_opencl_param_note);
	}
...
struct si_opencl_mem_t *si_opencl_mem_create()
{
	...
	/* Initialize */
	mem->id = si_opencl_object_new_id(SI_OPENCL_OBJ_MEM);
	mem->ref_count = 1;

	/* Return */
	si_opencl_object_add(mem);
	return mem;
}
...
struct si_opencl_event_t *si_opencl_event_create(enum si_opencl_event_kind_t kind)
{
	...
	/* Initialize */
	event->id = si_opencl_object_new_id(SI_OPENCL_OBJ_EVENT);
	event->ref_count = 1;
	event->kind = kind;

	/* Return */
	si_opencl_object_add(event);
	return event;
}
