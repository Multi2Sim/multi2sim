...
{
	this->ps_init_data = ps_init_data;
}
	...
	struct si_image_desc_t *image_desc, unsigned int uav)
{
	...
	/* Write the buffer resource descriptor into the UAV table */
	mem_write(emu->global_mem, this->uav_table +
		uav*EmuUavTableEntrySize, sizeof(*image_desc),
		image_desc);

	this->uav_table_entries[uav].valid = 1;
	this->uav_table_entries[uav].kind = SI_TABLE_ENTRY_KIND_IMAGE_DESC;
	this->uav_table_entries[uav].size = sizeof(*image_desc);
}
