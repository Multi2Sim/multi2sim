...
static int amd_opengl_shader_set_shader_kind(struct amd_opengl_shader_t *amd_opengl_shader)
{
	...
	internal_elf_file = amd_opengl_shader->internal_elf_file;
	external_elf_file = amd_opengl_shader->external_elf_file;
...
static struct amd_opengl_shader_t *amd_opengl_shader_create_from_buffer(void *ptr, int size, struct list_t *elf_offset_list, int elf_file_index, char *name)
{
	...
	amd_opengl_shader->external_elf_file = external_elf_file_create(ptr, size, elf_offset_list, elf_file_index, name);
	amd_opengl_shader->internal_elf_file = internal_elf_file_create(amd_opengl_shader->external_elf_file);
	if(!amd_opengl_shader_set_shader_kind(amd_opengl_shader))
		fatal("Set Shader kind fail!");
	if(!amd_opengl_shader_set_isa_buffer(amd_opengl_shader))
...
struct amd_opengl_bin_t *amd_opengl_bin_create(void *ptr, int size, char *name)
{
	...
	amd_opengl_bin->name = strdup(name);
	...
	amd_opengl_bin->shader_list = list_create();
	...
	{
		/* Initialize shader */
		amd_opengl_shader = amd_opengl_shader_create_from_buffer(ptr, size, elf_file_list, i, name);
		
		/* Add shader to shader list */
		list_add(amd_opengl_bin->shader_list,amd_opengl_shader);
	}
