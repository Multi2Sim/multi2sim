...
AddInstructionWorker::AddInstructionWorker(WorkItem *work_item,
...
AddInstructionWorker::~AddInstructionWorker()
{
}


template<typename T>
void AddInstructionWorker::Inst_ADD_Aux(BrigCodeEntry *instruction)
{
	T src0;
	T src1;
...
void AddInstructionWorker::Execute(BrigCodeEntry *instruction)
{
	...
	if (instruction->getKind() == BRIG_KIND_INST_BASIC)
	{
		...
		case BRIG_TYPE_S32:
			...
			Inst_ADD_Aux<int>(instruction);
			break;

		case BRIG_TYPE_S64:

			Inst_ADD_Aux<long long>(instruction);
			break;

		case BRIG_TYPE_U32:

			Inst_ADD_Aux<unsigned int>(instruction);
			break;
