...
#ifndef ARCH_KEPLER_TIMING_TIMING_H
#define ARCG_KEPLER_TIMING_TIMING_H
...
public:
	...
	public:
		...
		{
			AppendPrefix("Kepler timing simulator");
		}
	};
	...
	/// Flag indicates if the grid is finished to release driver call.
	bool grid_finished;

	void CreateGPU() { gpu = misc::new_unique<GPU>(); }
	...
	/// Run one iteration of the Kepler timing simulator. See
	...
	/// Check architecture-specific requirements for the memory configuration
	/// provided in the INI file. See comm::Timing::CheckMemoryCOnfiguration()
	/// for details
	...
	/// Dump a report of all the statistics collected during the execution of
	...
	/// See comm::Timing::getEntryModuel() for details.
	...
	/// Return number of available grids
	int getNumGrids() const { return grids.size(); }

	/// Return the first grid in the grids list
	Grid *getGrid()
	{
		if (grids.size() > 0)
			return grids.begin()->get();
		else
			return nullptr;
	}

	// Pop the front grid in the grids list
	void popGrid() { grids.pop_front(); }
};
