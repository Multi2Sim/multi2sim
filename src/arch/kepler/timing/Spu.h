...
#ifndef ARCH_KEPLER_TIMING_SPU_H
#define ARCH_KEPLER_TIMING_SPU_H
...
class SPU : public ExecutionUnit
{
	// Variable number of read instructions
	std::deque<std::unique_ptr<Uop>> read_buffer;

	// Variable number of execute instructions
...
public:
	...
	/// Constructor
	SPU(int id, SM *sm) : ExecutionUnit(sm), id(id)
	{
	}
	...
	/// Dispatch the given instruction into the SPU
