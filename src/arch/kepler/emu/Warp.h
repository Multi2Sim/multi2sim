...
CLASS_BEGIN(KplWarp, Object)
	...
	/* Sync stack */
	KplWarpSyncStackEntry new_entry;
	...
	/* Linked lists */
	KplWarp *running_list_next;
	KplWarp *running_list_prev;
	KplWarp *barrier_list_next;
	KplWarp *barrier_list_prev;
	KplWarp *finished_list_next;
	KplWarp *finished_list_prev;
...
#define KPL_FOREACH_WARP_IN_GRID(GRID, WARP_ID) \
	for ((WARP_ID) = (GRID)->warp_id_first; \
		(WARP_ID) <= (GRID)->warp_id_last; \
		(WARP_ID)++)
#define KPL_FOREACH_WARP_IN_THREADBLOCK(THREADBLOCK, WARP_ID) \
	for ((WARP_ID) = (THREADBLOCK)->warp_id_first; \
		(WARP_ID) <= (THREADBLOCK)->warp_id_last; \
		(WARP_ID)++)


void KplWarpCreate(KplWarp *self, KplThreadBlock *thread_block);
