...
#define KPL_MAX_LOCAL_MEM_ACCESSES_PER_INST  2
...
CLASS_BEGIN(KplThread, Object)
	...
	/* Thread state */
	KplThreadReg gpr[128];  /* General purpose registers */
	...
	unsigned int lds_access_addr[KPL_MAX_LOCAL_MEM_ACCESSES_PER_INST];
	unsigned int lds_access_size[KPL_MAX_LOCAL_MEM_ACCESSES_PER_INST];
	int lds_access_type[KPL_MAX_LOCAL_MEM_ACCESSES_PER_INST];  /* 0-none, 1-read, 2-write */

CLASS_END(KplThread)


#define KPL_FOREACH_THREAD_IN_GRID(GRID, THREAD_ID) \
	for ((THREAD_ID) = (GRID)->thread_id_first; \
		(THREAD_ID) <= (GRID)->thread_id_last; \
		(THREAD_ID)++)
#define KPL_FOREACH_THREAD_IN_THREADBLOCK(THREADBLOCK, THREAD_ID) \
	for ((THREAD_ID) = (THREADBLOCK)->thread_id_first; \
		(THREAD_ID) <= (THREADBLOCK)->thread_id_last; \
		(THREAD_ID)++)
#define KPL_FOREACH_THREAD_IN_WARP(WARP, THREAD_ID) \
...
/* void KplThreadUpdateBranchDigest(KplThread *self,
