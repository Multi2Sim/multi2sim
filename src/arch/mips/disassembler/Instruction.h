...
#ifndef ARCH_MIPS_DISASSEMBLER_INSTRUCTION_H
#define ARCH_MIPS_DISASSEMBLER_INSTRUCTION_H
...
public:
	...
	union Bytes
	{
		...
		BytesStandard standard;
		BytesTarget target;
		BytesOffsetImm offset_imm;
		BytesCC cc;
		BytesCode code;
		BytesSel sel;
...
public:
	...
	/// branch addresses.
	void Decode(unsigned int address, const char *buffer);
	...
	/// Getters
	std::string getName() const { return info ? info->name : "<unknwon>"; }
	Opcode getOpcode() const { return info ? info->opcode : OpcodeInvalid; }
