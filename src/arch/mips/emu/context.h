...
class MIPSContext {
    private:
        ...
        static unsigned int LoadAuxiliaryVector(MIPSContext_Impl *self, unsigned int where);
        static void LoadStack(MIPSContext_Impl *self);
    ...
    public:
        ...
        int GetState(MIPSContextState status);
        void SetState(MIPSContextState status);
        void ClearState(MIPSContextState status);

        void Execute();
        void GetFullPath(char *file_name, char *full_path, int size);
        void Finish(int status);
        void FinishGroup(int status);
        void GenerateProcSelfMaps(char *path);

        void AddArgsVector(int argc, char **argv);
        void AddEnviron(char *env);
        void LoadExecutable(char *path);
