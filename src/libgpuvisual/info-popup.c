...
void info_popup_show(char *text)
{
	...
	g_signal_connect(G_OBJECT(window), "button-press-event", G_CALLBACK(info_popup_button_press_event), NULL);
	g_signal_connect(G_OBJECT(window), "motion-notify-event", G_CALLBACK(info_popup_motion_event), NULL);
