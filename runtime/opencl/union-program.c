	...
	unsigned int length)
{
	...
	LIST_FOR_EACH(device->devices, i)
	{
		...
		/* go through all the binaries.  When one matches, use it to create a program for the current device */
		LIST_FOR_EACH(elf_file->section_list, j)
		{
			struct elf_section_t *section = list_get(elf_file->section_list, j);
			if (!strcmp(section->name, ".text"))
			{
				if (subdevice->arch_program_valid_binary_func(subdevice->arch_device, section->buffer.ptr, section->buffer.size))
				{
					list_add(
						program->programs, 
