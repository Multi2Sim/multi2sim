...
struct cuda_device_t *cuda_device_create(enum cuda_device_type_t dev_type)
{
	...
	if (dev_type == CUDA_DEVICE_FERMI)
	{
		...
		device->attributes[CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT] = 15;
		...
		device->attributes[CU_DEVICE_ATTRIBUTE_MEMORY_CLOCK_RATE] = 1848000;
		...
		device->attributes[CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR] = 2;
		device->attributes[CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR] = 0;
		device->attributes
		[CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE1D_MIPMAPPED_WIDTH] = 16384;
	}
	else if (dev_type == CUDA_DEVICE_KEPLER)
	{
		/* Tesla K20c */
