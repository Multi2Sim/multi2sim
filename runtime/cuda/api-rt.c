...
char *cuda_rt_err_not_impl = "\tMulti2Sim provides partial support for CUDA \
			      runtime library. To request the implementation \
			      of a certain functionality, please email \
			      development@multi2sim.org.\n";

char *cuda_rt_err_native = "\tYou are trying to run natively an application \
			    using the Multi2Sim CUDA runtime/driver library \
			    implementation ('libm2s-cuda'). Please run this \
			    program on top of Multi2Sim.\n";

char *cuda_rt_err_param_note = "\tThis error message is generated by the \
				Multi2Sim CUDA runtime library linked with \
				your CUDA host program. While a complete CUDA \
				implementation would return an error code to \
				your application, the Multi2Sim CUDA runtime \
				just makes your program fail with an error \
				message.\n";
...
cudaError_t cudaGetDeviceCount(int *count)
{
	...
	*count = 2;	
	...
	cuda_debug_print(stdout, "\t(runtime) out: count = [%p] %d\n", 
			count, *count);
...
cudaError_t cudaMalloc(void **devPtr, size_t size)
{
	...
	*(CUdeviceptr *)devPtr = dptr;
	...
	cuda_debug_print(stdout, "\t(runtime) out: devPtr = [%p] 0x%08x\n", 
