...
static struct str_map_t opencl_hsa_arg_type_map =
{
	4,
	{
		{ "value", opencl_hsa_arg_value },
		{ "pointer", opencl_hsa_arg_pointer },
		{ "image", opencl_hsa_arg_image },
		{ "sampler", opencl_hsa_arg_sampler }
	}
};
...
struct opencl_hsa_arg_t *opencl_hsa_arg_create(enum opencl_hsa_arg_type_t type,
		char *name)
{
	struct opencl_hsa_arg_t *arg;

	/* Initialize */
	arg = xcalloc(1, sizeof(struct opencl_hsa_arg_t));
	arg->type = type;
...
void opencl_hsa_arg_debug(struct opencl_hsa_arg_t *arg)
{
	opencl_debug("\ttype = %s", str_map_value(&opencl_hsa_arg_type_map,
			arg->type));
...
static void opencl_hsa_kernel_metadata_line(struct opencl_hsa_kernel_t *kernel,
	char *line)
{
	...
	else if (!strcmp(token, "pointer"))
	{
		...
		str_token_list_shift(token_list);
		token = str_token_list_first(token_list);
		arg = opencl_hsa_arg_create(opencl_hsa_arg_pointer, token);
	...
	else if (!strcmp(token, "value"))
	{
		...
		str_token_list_shift(token_list);
		token = str_token_list_first(token_list);
		arg = opencl_hsa_arg_create(opencl_hsa_arg_value, token);
		str_token_list_shift(token_list);
...
struct opencl_hsa_kernel_t *opencl_hsa_kernel_create(
	...
	char *func_name)
{
	struct opencl_hsa_kernel_t *kernel;
	...
	/* Initialize */
	kernel = xcalloc(1, sizeof(struct opencl_hsa_kernel_t));
	...
	while (elf_buffer.pos < elf_buffer.size)
	{
		...
		opencl_hsa_kernel_metadata_line(kernel, line);
	}
	...
	opencl_hsa_kernel_debug(kernel);
...
void opencl_hsa_kernel_free(struct opencl_hsa_kernel_t *kernel)
{
	...
	LIST_FOR_EACH(kernel->arg_list, index)
		opencl_shsa_rg_free(list_get(kernel->arg_list, index));
	...
	unsigned int *global_work_size, unsigned int *local_work_size)
{
	...
	assert(ndrange);
	assert(hsa_kernel);
	assert(hsa_kernel->type == opencl_runtime_type_si);

	opencl_debug("[%s] creating southern-islands ndrange", __FUNCTION__);

	arch_ndrange = (struct opencl_hsa_ndrange_t *)xcalloc(1,
		sizeof(struct opencl_hsa_ndrange_t));
	arch_ndrange->id = ndrange_count++;
	arch_ndrange->type = opencl_runtime_type_si;
	arch_ndrange->parent = ndrange;
	arch_ndrange->arch_kernel = hsa_kernel;
	...
	struct opencl_event_t *event)
{
	...
	opencl_hsa_ndrange_run_partial(ndrange, 0, ndrange->total_num_groups);

	/* Wait for the nd-range to complete and then flush the cache */
	opencl_hsa_ndrange_finish(ndrange);
	...
	/* Free the nd-range */
	opencl_hsa_ndrange_free(ndrange);
}
